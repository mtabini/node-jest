...
Client.prototype._connected = function(socket) {
  ...
  socket.__id = crypto.randomBytes(10).toString('hex');
...
Client.prototype._disconnect = function(socket) {
  this._sliceSocket(socket);

  var self = this;

  setTimeout(socket._retryPeriod, function() {
    if (socket._retryPeriod == 0) {
      socket._retryPeriod = 1000;
    } else {
      socket._retryPeriod *= 2;
    }

    this._connect(socket._config);
  });
...
Client.prototype.sendRequest = function(methodName, args, cb) {
  ...
  if (!socket) {
    ...
    deferred.reject(new Error('No connections available'));
